...
class StateStoreImpl {
  ...
  /**
   * Réinitialise la mémoire vive vers l'état d'usine (switch de compte).
   * N'efface PAS le localStorage d'un autre user — seulement le state RAM.
   */
  reset(): void {
    if (this.saveTimer) {
      clearTimeout(this.saveTimer);
    ...
    this.hydrated = true;
    this.listeners.forEach((l) => l(this.state));
    EventBus.publish(AppEvents.STATE_RESET, {
      at: new Date().toISOString(),
    });
    EventBus.publish(AppEvents.STATE_UPDATED, {
      path: 'reset',
      state: this.state,
    });
  }
  ...
  /**
   * Remplace l'état complet (re-hydratation cloud / merge).
   * Persiste immédiatement dans le scope actif.
   */
  replaceState(next: AppState, opts?: { persist?: boolean; path?: string }): void {
    const base = createInitialState();
    this.state = {
      ...base,
      ...next,
  ...
  /** Re-hydrate depuis le StorageRepository du scope courant */
  rehydrateFromScope(): AppState {
    return this.hydrate();
  }
}
