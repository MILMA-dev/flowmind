...
export const AppEvents = {
  ...
  // Multi-device / multi-compte sync
  GLOBAL_SYNC_STATUS: 'GLOBAL_SYNC_STATUS',
  GLOBAL_SYNC_PULLED: 'GLOBAL_SYNC_PULLED',
  GLOBAL_SYNC_PUSHED: 'GLOBAL_SYNC_PUSHED',
  GLOBAL_SYNC_REMOTE_CHANGE: 'GLOBAL_SYNC_REMOTE_CHANGE',
...
/** Profil utilisateur (sans secret) */
...
/** Profil étendu pour stockage hybride (local + distant) */
...
export interface ExtendedUserProfile extends UserProfile {
  /** Horodatage dernière écriture (local ou remote) */
  updatedAt: string;
  /** Dernière synchro réussie vers le distant */
  lastSyncedAt: string | null;
  /** Modifications locales non poussées */
  pendingSync: boolean;
  /** Origine de la dernière écriture */
  lastWriteSource?: 'local' | 'remote';
}

export interface StorageConfig {
  /** Préfixe clés local */
  localNamespace: string;
  /** Préfixe / table remote simulée */
  remoteNamespace: string;
  /** Latence simulée remote (ms) */
  remoteLatencyMs: number;
  /** Force offline pour tests */
  forceOffline?: boolean;
  /** URL API optionnelle (Supabase REST etc.) */
...
/** Snapshot cloud complet d'un utilisateur */
...
/** Payload temps réel (diff ou full) */
export interface SyncPayload {
  type: 'full' | 'patch' | 'ping';
  userId: string;
  deviceId: string;
  revision: number;
  updatedAt: string;
  /** Full snapshot ou sous-ensemble d'entités */
...
export interface AuthState {
  ...
  user: AuthUser | null;
  error: string | null;
  /** true si l'email doit être validé pour accéder à l'app */
...
export interface AuthResult {
  ...
  user?: AuthUser;
  session?: UserSession | null;
  /** Inscription : e-mail de vérif « envoyé » */
